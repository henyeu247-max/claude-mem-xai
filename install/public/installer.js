...
var MARKETPLACE_DIR = join3(homedir3(), ".claude", "plugins", "marketplaces", "henyeu247-max");
...
function registerMarketplace() {
  ...
  knownMarketplaces["henyeu247-max"] = {
    source: {
      source: "github",
      repo: "henyeu247-max/claude-mem"
    },
...
function registerPlugin(version) {
  ...
  const pluginCachePath = join3(PLUGINS_DIR, "cache", "henyeu247-max", "claude-mem", version);
  const now = (/* @__PURE__ */ new Date()).toISOString();
  installedPlugins.plugins["claude-mem@henyeu247-max"] = [
    {
      scope: "user",
...
function enablePluginInClaudeSettings() {
  ...
  settings.enabledPlugins["claude-mem@henyeu247-max"] = true;
  ...
  await Ye([
    {
      ...
      task: async (message) => {
        ...
        execSync3(
          `git clone --depth 1 https://github.com/henyeu247-max/claude-mem.git "${tempDir}"`,
          { stdio: "pipe" }
        );
