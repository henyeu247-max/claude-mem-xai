...
export const sessionInitHandler: EventHandler = {
  async execute(input: NormalizedHookInput): Promise<HookResult> {
    ...
    // Only initialize SDK agent for Claude Code (not Cursor/Windsurf)
    // Cursor/Windsurf don't use the SDK agent - they only need session/observation storage
    if (input.platform !== 'cursor' && input.platform !== 'windsurf' && sessionDbId) {
    ...
    } else if (input.platform === 'cursor' || input.platform === 'windsurf') {
