...
function isPluginDisabledInClaudeSettings() {
  try {
    ...
    return settings?.enabledPlugins?.['claude-mem@henyeu247-max'] === false;
  } catch {
    return false;
  }
}
 ...
 * 3. XDG path (~/.config/claude/plugins/marketplaces/henyeu247-max)
 * 4. Legacy path (~/.claude/plugins/marketplaces/henyeu247-max)
 */
function resolveRoot() {
  ...
  const marketplaceRel = join('plugins', 'marketplaces', 'henyeu247-max');
